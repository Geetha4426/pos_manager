...
class PolymarketClient:
    ...
    async def get_sports_events(self, sport: str, limit: int = 15) -> List[Event]:
        ...
        2. Each event contains multiple markets (sub-markets)
        3. Filter strictly by sport using keywords
        """
        events = []
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                # Approach 1: Use tag_slug if available
                params = {
                    "active": True,
                    "closed": False,
                ...
                if resp.status_code == 200:
                    ...
                    for item in data:
                        ...
                        if any(kw in combined for kw in sport_kws):
                            ...
                            # Only add if has sub-markets or is a single market event
                            if sub_markets or not raw_markets:
        ...
        # Fallback: If no events found, try markets endpoint with strict filtering
        if not events:
            markets = await self.get_sports_markets(sport=sport, limit=limit)
            for m in markets:
                events.append(Event(
    ...
    async def get_sports_markets(
        ...
        """
        Get sports markets with STRICT keyword filtering.
        No more Trump/Biden in cricket!
        """
        all_markets = []
